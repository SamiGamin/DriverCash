...
interface KilometrajeRecorridoDao {
    ...
    @Query("DELETE FROM kilometraje_recorrido") // "ingresos" es el nombre de tu tabla de Ingreso
    ...
    // Nueva función suspendida para obtener la lista de kilometrajes por ID de vehículo para el backup
    @Query("SELECT * FROM kilometraje_recorrido WHERE vehiculoId = :vehiculoId ORDER BY fecha DESC")
